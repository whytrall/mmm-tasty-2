class MainController < ApplicationController
  ...
  def faves
    ...
    unless friend_ids.blank?
      @entry_ids  = Fave.paginate :all, :select => 'faves.entry_id AS id', :conditions => "faves.user_id IN (#{friend_ids.join(',')})", :order => 'faves.id DESC', :page => @page, :per_page => Entry::PAGE_SIZE
      @entries    = Entry.find_all_by_id(@entry_ids.map(&:id), :include => [:rating, :author, { :attachments => :thumbnails }]).sort_by { |entry| @entry_ids.map(&:id).index(entry.id) }

      @comment_views = User::entries_with_views_for(@entry_ids.map(&:id), current_user)
    end
  ...
  def users
    @users = User.active_for(current_user, :limit => 20)
